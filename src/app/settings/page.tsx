...
import { Loader2, Save, Trash2, Plus, Wallet, Tags, AlertTriangle, ShieldCheck, Target, Calculator, Home, Briefcase } from "lucide-react";
  ...
  const fetchSettings = async () => {
    ...
    if (familyId) {
      const { data, error } = await supabase
        .from('families')
        .select('expense_categories, income_categories, expected_monthly_income, budget_limits, business_expense_categories, business_income_categories')
        .eq('id', familyId)
        .single();

      if (data && !error) {
        // Family Data (Using ?? [] so it respects if you intentionally delete everything)
        setFamilyExpenseCats(data.expense_categories ?? []);
        setFamilyIncomeCats(data.income_categories ?? []);
        setExpectedIncome(data.expected_monthly_income || 0);
        setBudgetLimits(data.budget_limits ?? {});

        // Business Data
        setBusinessExpenseCats(data.business_expense_categories ?? ["Software", "Marketing", "Contractors"]);
        setBusinessIncomeCats(data.business_income_categories ?? ["Client Invoices", "Product Sales"]);
      }
    }
    setIsLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    if (familyId) {
      const { error } = await supabase
        .from('families')
        .update({
          expense_categories: familyExpenseCats,
          income_categories: familyIncomeCats,
          expected_monthly_income: expectedIncome,
          budget_limits: budgetLimits,
          business_expense_categories: businessExpenseCats,
          business_income_categories: businessIncomeCats
        })
        .eq('id', familyId);
    ...
    if (btn) {
      ...
      setTimeout(() => {
        ...
        btn.classList.add(activeTab === 'business' ? 'bg-emerald-600' : 'bg-indigo-600');
      }, 2000);
    }
  };
  ...
  return (
    <div className="relative min-h-[100dvh] pb-[calc(env(safe-area-inset-bottom)+8rem)] pt-[calc(env(safe-area-inset-top)+1rem)] [-webkit-tap-highlight-color:transparent]">
      ...
      <div className="px-4 md:px-8 max-w-2xl mx-auto pt-4 space-y-6">
        ...
        <div className="flex overflow-x-auto hide-scrollbar gap-2 bg-gray-200/50 dark:bg-gray-800/50 p-1.5 rounded-2xl snap-x">
          <button onClick={() => setActiveTab("family")} className={`flex-1 min-w-[100px] py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${activeTab === 'family' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>
            <Home size={16}/> Family
          </button>
          <button onClick={() => setActiveTab("business")} className={`flex-1 min-w-[100px] py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${activeTab === 'business' ? 'bg-white dark:bg-gray-700 shadow-sm text-emerald-600 dark:text-emerald-400' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>
            <Briefcase size={16}/> Business
          </button>
          <button onClick={() => setActiveTab("wallets")} className={`flex-1 min-w-[100px] py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${activeTab === 'wallets' ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>
            <Wallet size={16}/> Accounts
          </button>
          <button onClick={() => setActiveTab("danger")} className={`flex-1 min-w-[100px] py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${activeTab === 'danger' ? 'bg-white dark:bg-gray-700 shadow-sm text-rose-600 dark:text-rose-400' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>
        ...
        <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-gray-100 dark:border-gray-800 p-5 md:p-6 rounded-[2rem] shadow-sm min-h-[50vh] transform-gpu">
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
                ...
                <div className="relative z-10">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="relative">
                      ...
                      <input type="number" value={expectedIncome || ""} onChange={(e) => setExpectedIncome(Number(e.target.value))} className="w-full bg-white/10 backdrop-blur-md border border-white/20 rounded-xl py-3 pl-12 pr-4 outline-none focus:ring-2 focus:ring-white/50 text-white font-black text-xl placeholder-white/30" placeholder="0" />
                    </div>
                  </div>
                  <div className="bg-white/10 rounded-2xl p-4 backdrop-blur-md border border-white/10">
                    <div className="flex justify-between text-sm mb-2 font-bold">
                      <span className="text-indigo-100">Total Allocated</span>
                      <span className={leftToAllocate < 0 ? "text-rose-300" : "text-white"}>{totalBudgeted.toLocaleString()} Ks</span>
                    </div>
                    ...
                    <div className="flex justify-between text-xs mt-2 font-medium">
                      ...
                      <span className={leftToAllocate < 0 ? "text-rose-300 font-bold" : "text-emerald-300 font-bold"}>{leftToAllocate.toLocaleString()} Ks</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-4">
                  <input type="text" placeholder="e.g., Rent" value={newFamilyExp} onChange={e => setNewFamilyExp(e.target.value)} onKeyDown={e => e.key === 'Enter' && addFamilyExpense()} className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-rose-500 text-sm" />
                  <button onClick={addFamilyExpense} className="bg-rose-100 dark:bg-rose-900/30 text-rose-600 p-3 rounded-xl active:scale-95"><Plus size={20} /></button>
                </div>
                    ...
                    <div key={cat} className="flex flex-col sm:flex-row sm:items-center justify-between bg-gray-50 dark:bg-gray-800 p-3 rounded-2xl border border-gray-100 dark:border-gray-700 gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="relative w-full sm:w-40">
                          ...
                          <input type="number" value={budgetLimits[cat] || ""} onChange={(e) => setBudgetLimits(prev => ({ ...prev, [cat]: Number(e.target.value) }))} placeholder="Limit" className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl py-2 pl-9 pr-3 text-sm font-bold text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 text-right" />
                        </div>
                        <button onClick={() => removeFamilyExpense(cat)} className="p-2 text-gray-400 hover:text-rose-500 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
                ...
                <div className="flex gap-2 mb-4">
                  <input type="text" placeholder="e.g., Salary" value={newFamilyInc} onChange={e => setNewFamilyInc(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem(newFamilyInc, setFamilyIncomeCats, () => setNewFamilyInc(""))} className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500 text-sm" />
                  <button onClick={() => addItem(newFamilyInc, setFamilyIncomeCats, () => setNewFamilyInc(""))} className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 p-3 rounded-xl active:scale-95"><Plus size={20} /></button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {familyIncomeCats.map(cat => (
                    <div key={cat} className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 px-3 py-1.5 rounded-lg flex items-center gap-2 text-sm font-medium border border-emerald-100 dark:border-emerald-800/50">
                      {cat} <button onClick={() => removeItem(cat, setFamilyIncomeCats)} className="text-emerald-400/50 hover:text-emerald-500"><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeTab === "business" && (
            <div className="space-y-8 animate-in fade-in duration-300">
              ...
              <div>
                ...
                <div className="flex gap-2 mb-4">
                  <input type="text" placeholder="e.g., Server Hosting" value={newBusinessExp} onChange={e => setNewBusinessExp(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem(newBusinessExp, setBusinessExpenseCats, () => setNewBusinessExp(""))} className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-cyan-500 text-sm" />
                  <button onClick={() => addItem(newBusinessExp, setBusinessExpenseCats, () => setNewBusinessExp(""))} className="bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 p-3 rounded-xl active:scale-95"><Plus size={20} /></button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {businessExpenseCats.map(cat => (
                    <div key={cat} className="bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-300 px-3 py-2 rounded-lg flex items-center gap-2 text-sm font-bold border border-cyan-100 dark:border-cyan-800/50">
                      {cat} <button onClick={() => removeItem(cat, setBusinessExpenseCats)} className="text-cyan-400/50 hover:text-cyan-500"><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
                ...
                <div className="flex gap-2 mb-4">
                  <input type="text" placeholder="e.g., Client Retainers" value={newBusinessInc} onChange={e => setNewBusinessInc(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem(newBusinessInc, setBusinessIncomeCats, () => setNewBusinessInc(""))} className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500 text-sm" />
                  <button onClick={() => addItem(newBusinessInc, setBusinessIncomeCats, () => setNewBusinessInc(""))} className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 p-3 rounded-xl active:scale-95"><Plus size={20} /></button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {businessIncomeCats.map(cat => (
                    <div key={cat} className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 px-3 py-2 rounded-lg flex items-center gap-2 text-sm font-bold border border-emerald-100 dark:border-emerald-800/50">
                      {cat} <button onClick={() => removeItem(cat, setBusinessIncomeCats)} className="text-emerald-400/50 hover:text-emerald-500"><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeTab === "wallets" && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="flex gap-2 mb-4">
                <input type="text" placeholder="e.g., KBZPay" value={newWallet} onChange={e => setNewWallet(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem(newWallet, setWallets, () => setNewWallet(""))} className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
                <button onClick={() => addItem(newWallet, setWallets, () => setNewWallet(""))} className="bg-blue-100 dark:bg-blue-900/30 text-blue-600 p-3 rounded-xl active:scale-95"><Plus size={20} /></button>
              </div>
              <div className="flex flex-col gap-2">
                {wallets.map(wallet => (
                  <div key={wallet} className="bg-gray-50 dark:bg-gray-800 px-4 py-3 rounded-xl flex items-center justify-between text-sm font-bold text-gray-700 dark:text-gray-300 border border-gray-100 dark:border-gray-700">
                    {wallet} <button onClick={() => removeItem(wallet, setWallets)} className="text-gray-400 hover:text-rose-500 p-2"><Trash2 size={16} /></button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
                {!showDisconnectConfirm ? (
                  <button onClick={() => setShowDisconnectConfirm(true)} className="w-full py-4 rounded-2xl border-2 border-dashed border-rose-200 dark:border-rose-900/50 text-rose-500 font-bold active:scale-95 transition-all">
                    Disconnect Device
                  </button>
                ) : (
                  <div className="bg-rose-50 dark:bg-rose-900/20 p-6 rounded-[2rem] border border-rose-200 dark:border-rose-500/30 animate-in zoom-in-95 duration-200">
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setShowDisconnectConfirm(false)} className="flex-1 py-3.5 bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-bold rounded-xl active:scale-95 border border-gray-200 dark:border-gray-700">Cancel</button>
                      <button onClick={handleDisconnect} className="flex-1 py-3.5 bg-rose-600 text-white font-bold rounded-xl active:scale-95">Disconnect</button>
                    </div>
                  </div>
                )}
              </div>
        ...
        <button 
          ...
          className={`w-full text-white font-bold py-4 rounded-2xl shadow-lg active:scale-95 transition-all flex justify-center items-center gap-2 transform-gpu ${activeTab === 'business' ? 'bg-emerald-600 shadow-emerald-500/20' : 'bg-indigo-600 shadow-indigo-500/20'}`}
        >
