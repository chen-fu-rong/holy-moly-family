...
export default function Dashboard() {
  ...
  return (
    ...
    >
      ...
      <div className="px-3 md:px-8 max-w-5xl mx-auto pt-3 md:pt-2 space-y-5 md:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4 md:mb-6">
          <div className="flex bg-gray-200/50 dark:bg-gray-800/50 p-1.5 rounded-2xl shadow-inner">
            <button 
              onClick={() => {
                setWorkspace("personal");
                triggerHaptic('light');
              }} 
              className={`py-2 md:py-2 px-3 md:px-4 rounded-lg text-xs md:text-sm font-bold transition-all flex items-center justify-center gap-1.5 ${!isBusiness ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
            >
              <Home size={16} /> <span>Family</span>
            </button>
            <button 
              onClick={() => {
              ...
              className={`py-2 md:py-2 px-3 md:px-4 rounded-lg text-xs md:text-sm font-bold transition-all flex items-center justify-center gap-1.5 ${isBusiness ? 'bg-white dark:bg-gray-700 shadow-sm text-emerald-600 dark:text-emerald-400' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
            >
          ...
          <div className="flex gap-2.5 w-full sm:w-auto">
            ...
            <Link 
              ...
              className="flex-1 sm:flex-none p-2.5 md:p-3 bg-white/60 dark:bg-gray-900/60 backdrop-blur-md border border-amber-200 dark:border-amber-900/30 rounded-2xl shadow-sm hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors text-amber-600 dark:text-amber-500 active:scale-95 flex items-center justify-center"
            >
            ...
            <Link href="/settings" className="flex-1 sm:flex-none p-2.5 md:p-3 bg-white/60 dark:bg-gray-900/60 backdrop-blur-md border border-gray-200 dark:border-gray-700/50 rounded-2xl shadow-sm hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-gray-600 dark:text-gray-300 active:scale-95 flex items-center justify-center">
              <Settings size={24} />
            </Link>
          </div>
        </div>
        ...
        >
          <div className={`col-span-2 md:col-span-2 rounded-2xl md:rounded-[2rem] p-5 md:p-6 text-white shadow-lg md:shadow-xl relative overflow-hidden transform-gpu transition-colors duration-700 ${isBusiness ? 'bg-gradient-to-br from-emerald-600 to-cyan-700' : 'bg-gradient-to-br from-indigo-600 to-fuchsia-700'}`}>
            <div className="relative z-10">
              <p className="text-white/80 font-bold text-xs md:text-sm flex items-center gap-2 mb-2 uppercase tracking-wider">
                {isBusiness ? <Briefcase size={14} /> : <Wallet size={14} />} 
                {isBusiness ? 'Business Net Profit' : 'Personal Liquidity'}
              </p>
              <h2 className="text-3xl md:text-5xl font-black tracking-tight mb-4 drop-shadow-sm">
                {activeBalance.toLocaleString()} <span className="text-lg md:text-xl text-white/60">{currency}</span>
              </h2>
            ...
            {isBusiness ? (
              <Briefcase size={100} className="absolute -bottom-4 -right-4 md:-bottom-6 md:-right-6 text-white/10 -rotate-12" />
            ) : (
              <Wallet size={100} className="absolute -bottom-4 -right-4 md:-bottom-6 md:-right-6 text-white/10 -rotate-12" />
            )}
          </div>

          {/* Today's In/Out */}
          <div className="col-span-1 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border border-gray-100 dark:border-gray-800 rounded-2xl md:rounded-[2rem] p-4 md:p-5 shadow-sm flex flex-col justify-between">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-3 ${isBusiness ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400' : 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400'}`}>
              <ArrowUpRight size={20} strokeWidth={3} />
            </div>
            <div>
              <p className="text-xs md:text-sm font-bold text-gray-500 uppercase tracking-wider mb-1">In (Today)</p>
              <p className="text-base md:text-lg font-extrabold text-gray-900 dark:text-white">+{activeIncome.toLocaleString()}</p>
            </div>
          </div>
          ...
          <div className="col-span-1 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border border-gray-100 dark:border-gray-800 rounded-2xl md:rounded-[2rem] p-4 md:p-5 shadow-sm flex flex-col justify-between">
            ...
            <div>
              <p className="text-xs md:text-sm font-bold text-gray-500 uppercase tracking-wider mb-1">Out (Today)</p>
              <p className="text-base md:text-lg font-extrabold text-gray-900 dark:text-white">-{activeExpense.toLocaleString()}</p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-4 pt-2 md:pt-4 pb-8">
                  ...
                  >
                    <div className="flex items-center gap-3 md:gap-4 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="font-bold text-gray-900 dark:text-white text-sm md:text-base line-clamp-1">{tx.category}</h4>
                        <p className="text-xs md:text-sm text-gray-500 font-medium line-clamp-1">
                          {tx.account || 'Cash'} • {tx.spender}
                        </p>
                        {tx.notes && (
                          <p className="text-[11px] md:text-xs text-gray-400 italic mt-0.5 line-clamp-1">
                            {tx.notes}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                      <div className="text-right">
                        <p className={`font-black text-sm md:text-base ${tx.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-900 dark:text-white'}`}>
                          {tx.type === 'income' ? '+' : '-'}{Number(tx.amount).toLocaleString()} {currency}
                        </p>
                        <p className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wide mt-1">
